...
async function openMicroservicesInBrowser(rootPath: string) {
...
async function deployDockerContainers(rootPath: string): Promise<void> {
    ...
    await openMicroservicesInBrowser(rootPath);
}

async function startDockerContainers(rootPath: string): Promise<void> {
...
export async function stopDockerContainers() {
    try {
        console.log('Stop Docker Containers command executed');
        ...
        if (!anyContainersRunning) {
            ...
            if (imagesBuilt) {
                ...
                if (startContainers === 'Start Containers') {
                    await startDockerContainers(rootPath);
                }
            } else {
                ...
                if (deployContainers === 'Deploy Containers') {
                    await deployDockerContainers(rootPath);
                }
            }
            return;
        }
        ...
        try {
            await execPromise('docker-compose down', { cwd: path.join(rootPath, 'deployment') });
            vscode.window.showInformationMessage('Docker containers have been successfully stopped !');
            console.log('Docker containers stopped successfully.');
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            vscode.window.showErrorMessage(`Failed to stop Docker containers: ${errorMessage}`);
            console.error(`Error: ${errorMessage}`, error);
        }
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
